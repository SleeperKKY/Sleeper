...
void onReceiveMessage(uint8_t* buffer, int msglen){
  ...
  if(reqMsg.getDeviceID()==TEMP_ID)//if device id for temperature sensor
  {          				
        if(reqMsg.getCommand()==TEMP_MEASURE_TEMPERATURE)
        {
		// measure temperature
                //resData[0]=getTemperature() ;	//if connected real temperature sensor
                resData[0]=23 ;//fake data just to make response message
	}
