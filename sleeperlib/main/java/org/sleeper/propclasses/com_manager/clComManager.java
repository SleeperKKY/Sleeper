...
public class clComManager{
    ...
    private final List<clComManagerThreadTask> taskList =new CopyOnWriteArrayList<>() ;
    ...
    private class clComManagerThreadTask implements Runnable {
        ...
        public void run() {
            ...
            if (needRespMsgQueue.poll()) {
                ...
                if (reqMsg.getDeviceID() == clAccelTempDataProcessor.TEMP_ID) {

                    rcvStream = (char) clResponseMessage.RES + "\n" + (char) clResponseMessage.SUCCESS + "\n" +
                            (char) clAccelTempDataProcessor.TEMP_ID + "\n" + (char) clAccelTempDataProcessor.TEMP_MEASURE_TEMPERATURE +
                            "" + (char) 35 + "\n";

                } else if (reqMsg.getDeviceID() == clAccelTempDataProcessor.FAN_ID) {

                    if (reqMsg.getCommand() == clAccelTempDataProcessor.FAN_SET_PWM) {

                        rcvStream = (char) clResponseMessage.RES + "\n" + (char) clResponseMessage.SUCCESS + "\n" +
                                (char) clAccelTempDataProcessor.FAN_ID + "\n" + (char) clAccelTempDataProcessor.FAN_SET_PWM +
                                "" + (char)reqMsg.getData()[0] + "\n";
                    }
...
//code for real, do not erase
                        ...
                        if(needRespMsgQueue.poll()) {
                            ...
                            while (timeoutCnt != timeout_count) {
                                ...
                                try {
                                    ...
                                    len=diStream.read(buf) ;//read byte array in buffer
                                    rcvStream=new String(buf,0,len) ;
                            ...
                            if (responseReceived) {
                                ...
                                final org.sleeper.propclasses.com_manager.clResponseMessage resMsg = new org.sleeper.propclasses.com_manager.clResponseMessage();
    ...
    public interface IMessageListener {
         ...
         */
        void onReceiveMessageEvent(org.sleeper.propclasses.com_manager.clResponseMessage ResponseMessage);
    }
}
